...
export default class Frames {
    ...
    _drawMiniFrame() {
        this.canvas.addEventListener(this.events.mouseup, this._drawMiniFrameEvent.bind(this));
    }
    ...
    _drawMiniFrameEvent() {
        ...
        selectedFrameCanvasCtx.clearRect(0, 0,
            selectedFrameCanvas.width, selectedFrameCanvas.height);
        img.src = currentCanvas;
        img.onload = () => {
            selectedFrameCanvasCtx.imageSmoothingEnabled = false;
            selectedFrameCanvasCtx.drawImage(img, 0, 0,
                selectedFrameCanvas.width, selectedFrameCanvas.height);
        };
    ...
    _addNewFrame() {
        ...
        addFrameButton.addEventListener(this.events.click, this._addNewFrameEvent.bind(this));
    ...
    _checkByKey(event) {
        ...
        const deleteFrameElement = document.querySelector(".frames__frame_selected")
            .querySelector(".frame__delete");
        const copyFrameElement = document.querySelector(".frames__frame_selected")
    ...
    _addSelectingFrame() {
        this.frameContainer.addEventListener(this.events.click,
    ...
    _deleteFrame(currentButton) {
        ...
        let frame = [...this.frameContainer.children].find((frameEl) => frameEl.classList.contains("frames__frame_selected") === true).querySelector(".frames__canvas");
        this.frames.splice(number, 1);

        if (currentButton.parentElement.classList.contains("frames__frame_selected")) {
            if (currentButton.parentElement.previousElementSibling) {
                currentButton.parentElement.previousElementSibling.classList.add("frames__frame_selected");
                frame = this.frames[currentButton
                    .parentElement.previousElementSibling.dataset.number];
            } else {
                currentButton.parentElement.nextElementSibling.classList.add("frames__frame_selected");
        ...
        if (this.frames.length === 1) {
            ...
            frameAlone.querySelector(".frame__delete").classList.remove("frame__visible");
            frameAlone.querySelector(".frame__move").classList.remove("frame__visible");
        }
    }
    ...
    _copyFrame(currentButton) {
        if (this.frames.length === 1) {
            ...
            frameAlone.querySelector(".frame__delete").classList.add("frame__visible");
