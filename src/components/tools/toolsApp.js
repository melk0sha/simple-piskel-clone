...
export default class ToolsApp {
  ...
  _initTools() {
    ...
    this.penSizeElement.addEventListener(
      this.pen.events.click,
      this._changePenSizeElement.bind(this)
    );
    this.tools.addEventListener(
      this.pen.events.click,
      this.setActiveTool.bind(this)
    );
    document.addEventListener(
      this.pen.events.keyup,
      this.setActiveTool.bind(this)
    );
  }
  ...
  _initHandlers() {
    ...
    this.colorSelectActive = this.colorSelect.colorSelectActive.bind(
      this.colorSelect
    );

    this.paintBucketActive = this.paintBucket.paintBucketActive.bind(
      this.paintBucket
    );
  ...
  setActiveTool(event) {
    ...
    const tool = [...this.tools.children].find(
      (toolElement) =>
        toolElement.classList.contains("tools__tool_active") === true
    );

    if (tool === this.pen.getElement()) {
      this._clearEventListeners();
      this.canvas.addEventListener(
        this.pen.events.mousedown,
        this.drawMouseDown
      );
      this.canvas.addEventListener(
        this.pen.events.mousemove,
        this.drawMouseMove
      );
      ...
      this._clearEventListeners();
      this.canvas.addEventListener(
        this.colorSelect.events.mousedown,
        this.colorSelectActive
      );
    } else if (tool === this.paintBucket.getElement()) {
      this._clearEventListeners();
      this.canvas.addEventListener(
        this.paintBucket.events.mousedown,
        this.paintBucketActive
      );
    } else if (tool === this.eraser.getElement()) {
      this._clearEventListeners();
      this.canvas.addEventListener(
        this.eraser.events.mousedown,
        this.eraserMouseDown
      );
      this.canvas.addEventListener(
        this.eraser.events.mousemove,
        this.eraserMouseMove
      );
      this.canvas.addEventListener(
        this.eraser.events.mouseup,
        this.eraserMouseUp
      );
    } else if (tool === this.paintSwap.getElement()) {
      this._clearEventListeners();
      this.canvas.addEventListener(
        this.paintSwap.events.mousedown,
        this.paintSwapActive
      );
    } else if (tool === this.stroke.getElement()) {
      this._clearEventListeners();
      this.canvas.addEventListener(
        this.stroke.events.mousedown,
        this.strokeMouseDown
      );
      this.canvas.addEventListener(
        this.stroke.events.mousemove,
        this.strokeMouseMove
      );
      this.canvas.addEventListener(
        this.stroke.events.mouseup,
        this.strokeMouseUp
      );
    }
  }
