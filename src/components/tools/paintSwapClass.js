...
export default class PaintSwap extends Tool {
  ...
  initPaintSwap() {
    ...
    paintSwap.addEventListener(
      this.events.click,
      this._selectPaintSwap.bind(this)
    );
  }
  ...
  _paintSwap(position) {
    ...
    const targetColorData = this.ctx.getImageData(
      Math.floor(x / (canvasStyleResolution / this.canvas.height)),
      Math.floor(y / (canvasStyleResolution / this.canvas.width)),
      1,
      1
    ).data;
