export default class LocalStorage {
  ...
  _initializeUserSettings() {
    this.previousSessionKeys = JSON.parse(
      localStorage.getItem(this.items.keys)
    );
    this.previousSessionFPS = localStorage.getItem(this.items.fps);
    this.previousSessionColors = JSON.parse(
      localStorage.getItem(this.items.colors)
    );
    ...
    this.previousSessionCanvasResolution = localStorage.getItem(
      this.items.canvasResolution
    );
  ...
  _updateKeys() {
    this.keyUpdate(
      document.querySelector(".pen-key"),
      this.previousSessionKeys.penKey
    );
    this.keyUpdate(
      document.querySelector(".color-select-key"),
      this.previousSessionKeys.colorSelectKey
    );
    this.keyUpdate(
      document.querySelector(".paint-bucket-key"),
      this.previousSessionKeys.paintBucketKey
    );
    this.keyUpdate(
      document.querySelector(".eraser-key"),
      this.previousSessionKeys.eraserKey
    );
    this.keyUpdate(
      document.querySelector(".paint-swap-key"),
      this.previousSessionKeys.paintSwapKey
    );
    this.keyUpdate(
      document.querySelector(".stroke-key"),
      this.previousSessionKeys.strokeKey
    );
    this.keyUpdate(
      document.querySelector(".swap-colors-key"),
      this.previousSessionKeys.colorSwapKey
    );
    this.keyUpdate(
      document.querySelector(".full-screen-key"),
      this.previousSessionKeys.previewFullScreenKey
    );
    this.keyUpdate(
      document.querySelector(".add-new-frame-key"),
      this.previousSessionKeys.addNewFrame
    );
    this.keyUpdate(
      document.querySelector(".delete-frame-key"),
      this.previousSessionKeys.deleteFrame
    );
    this.keyUpdate(
      document.querySelector(".copy-frame-key"),
      this.previousSessionKeys.copyFrame
    );
    this.keyUpdate(
      document.querySelector(".clear-canvas-key"),
      this.previousSessionKeys.clearCanvas
    );
    this.keyUpdate(
      document.querySelector(".export-gif-key"),
      this.previousSessionKeys.exportGIF
    );
    this.keyUpdate(
      document.querySelector(".export-apng-key"),
      this.previousSessionKeys.exportAPNG
    );
  }
  ...
  _updateColors() {
    ...
    this.colors.updateSecondaryColorElement(
      this.previousSessionColors.secondary
    );
  }

  _updateTool() {
    const updateToolElement = document.querySelector(
      `.tools__${this.previousSessionTool}`
    );
  ...
  _updatePenSize() {
    const updatePenSizeElement = document.querySelector(
      `.pen-size__${this.previousSessionPenSize}pixel`
    );
  ...
  _updateCanvasResolution() {
    const updateResolutionElement = document.querySelector(
      `.canvas-size__btn__${this.previousSessionCanvasResolution}x${this.previousSessionCanvasResolution}`
    );
  ...
  _saveBeforeExit() {
    window.addEventListener(this.events.beforeunload, () => {
      ...
      const penSize = document.querySelector(".pen-size__option_active").dataset
        .value;
      const canvasResolution = document.querySelector(
        ".canvas-size__btn_active"
      ).dataset.resolution;
      ...
      localStorage.setItem(
        this.items.colors,
        JSON.stringify(this.colors.colors)
      );
